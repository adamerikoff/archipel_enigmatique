defmodule Extrait.Gatherer do
  ...
  def handle_cast({:result, path, hash}, worker_count) do
    Extrait.Results.add_hash_for(path, hash)
  ...
  def handle_info(:kickoff, worker_count) do
    1..worker_count
    |> Enum.each(fn _ -> Extrait.WorkerSupervisor.add_worker() end)
  ...
  defp report_results() do
    ...
    Extrait.Results.find_duplicates |> Enum.each(&IO.inspect/1)
  end
end
